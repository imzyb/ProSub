...
// --- 全局配置 ---
const SUBVERTER_URL = 'api.v1.mk';
const PREDEFINED_RULES = {
    Lan: ['FINAL,DIRECT,dns-failed'], Apple: ['RULE-SET,apple,DIRECT'], Microsoft: ['RULE-SET,microsoft,DIRECT'],
    Google: ['RULE-SET,google,🚀 PROXY'], Proxy: ['RULE-SET,proxy,🚀 PROXY'], Cn: ['RULE-SET,cn,DIRECT'],
    Telegram: ['RULE-SET,telegram,🚀 PROXY'], Private: ['RULE-SET,private,DIRECT'], Domestic: ['RULE-SET,domestic,DIRECT'],
...
export async function onRequest(context) {
    ...
    if (resource === 'subscribe' && id) {
        try {
            ...
            if (resolvedNodes.length === 0) return new Response(`// No nodes found`, { status: 200 });

            const format = targetProfile.outputFormat.toLowerCase();
            if (format === 'clash' || format === 'v2ray' || format === 'v2rayn') {
                let outputConfig = '';
                let contentType = 'text/plain; charset=utf-8';
                let fileExtension = 'txt';
                if (format === 'v2ray') { outputConfig = buildV2rayConfig(resolvedNodes); contentType = 'application/json; charset=utf-8'; fileExtension = 'json'; }
                else if (format === 'v2rayn') { outputConfig = btoa(resolvedNodes.map(n => n.url).join('\n')); }
                else { outputConfig = buildClashConfig(resolvedNodes, targetProfile); fileExtension = 'yaml'; }
                ...
                const nodeString = resolvedNodes.map(n => n.url).join('\n');
                const base64Nodes = btoa(nodeString);
    ...
    const handleCrud = async (kvKey, request, id) => {
        ...
        switch (request.method) {
            ...
            case 'POST': { const newItems = await request.json(); await env.KV.put(kvKey, JSON.stringify(newItems)); return new Response(JSON.stringify({ success: true })); }
            case 'PUT': { if (!id) return new Response('ID required', { status: 400 }); const updatedItem = await request.json(); const itemIndex = items.findIndex(item => item.id === id); if (itemIndex === -1) return new Response('Not found', { status: 404 }); items[itemIndex] = { ...updatedItem, id }; await env.KV.put(kvKey, JSON.stringify(items)); return new Response(JSON.stringify(items[itemIndex])); }
            case 'DELETE': { if (!id) return new Response('ID required', { status: 400 }); const remaining = items.filter(item => item.id !== id); await env.KV.put(kvKey, JSON.stringify(remaining)); return new Response(null, { status: 204 }); }
